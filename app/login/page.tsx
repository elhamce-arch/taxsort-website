...
export default function LoginPage() {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center" style={{ background: "#F0F6F5" }}><div className="w-8 h-8 border-2 border-teal-600 border-t-transparent rounded-full animate-spin" /></div>;
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4" style={{ background: "#F0F6F5" }}>
      <div className="bg-white rounded-2xl shadow-lg p-10 w-full max-w-sm flex flex-col items-center gap-6">
        ...
        {error && <p className="text-sm text-red-600 bg-red-50 rounded-lg px-4 py-2 w-full text-center">{error}</p>}
        ...
        <p className="text-xs text-gray-400 text-center">
          Your data is the same as your TaxSort mobile app
        </p>
      </div>
    </div>
  );
